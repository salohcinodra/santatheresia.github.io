...
function the_schema_save_sidebar_layout( $post_id ){
    global $the_schema_sidebar_layout;
    ...
    $layout = isset( $_POST['the_schema_sidebar_layout'] ) ? sanitize_key( $_POST['the_schema_sidebar_layout'] ) : 'default-sidebar';

    if( array_key_exists( $layout, $the_schema_sidebar_layout ) ){
        update_post_meta( $post_id, '_the_schema_sidebar_layout', $layout );
    }else{
        delete_post_meta( $post_id, '_the_schema_sidebar_layout' );
    }
}
